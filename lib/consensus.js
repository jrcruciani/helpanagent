...
export function computeConsensus(responses, respondents) {
  ...
  for (const r of respondents) {
    // Only use earned reputation if respondent has completed enough calibrations
    const isCalibrated = (r.calibration_count || 0) >= MIN_CALIBRATION;
    reputationMap[r.token] = isCalibrated ? r.reputation_score : 1.0;
  }
